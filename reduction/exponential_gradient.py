...
def predict_proba(X, estimators_, weights = None):
    ...
    avg_probas = np.mean(probas * weights, axis=0)
    return avg_probas
...
def get_group_wise_loss(X, y, estimators_, weights):
...
# def get_group_wise_error(X, y, a, estimators_):
#     p = predict(X, estimators_)
#     loss = (p != y).astype('float')
    

def update_theta(X, y, estimators_, lagrangian_constants_, weights, eps):
...
# def reduction_update(max_iter, n_estimators, base_estimator,
#                      random_state, weights, lagrangian_constants_):
#     flag = 0
#     step = 0
