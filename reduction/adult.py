...
def get_adult_data(path = "./dataset/adult.data"):
    '''
    ...
    train = pd.read_csv(os.path.join(path, "adult.data"), header = None)
    test = pd.read_csv(os.path.join(path, "adult.test"), header = None)
...
def preprocess_adult_data(seed = 0, path = "./dataset/adult.data"):
    '''
    ...
    dataset_orig = get_adult_data(path = path)
