...
def demographic_metrics(y_true, y_pred, y_protected):
    ...
    idx_bad_class = np.where(y_true != 1)[0]
    bal_acc = (correct[idx_good_class].mean() + correct[idx_bad_class].mean())/2
    print('Balanced accuracy is %f' % bal_acc)
    print("Demographic parity difference is", np.abs(correct_pos - correct_neg))
    print("Demographic parity ratio is", max(correct_pos / correct_neg, correct_neg / correct_pos))
...
def EO_metrics(y_true, y_pred, y_protected, label_protected=0, label_good=0):
    idx_prot = np.where(y_protected == label_protected)[0]
    idx_priv = np.where(y_protected != label_protected)[0]
    idx_good_class = np.where(y_true == label_good)[0]
    idx_pred_good_class = np.where(y_pred == label_good)[0]
    ...
    return bal_acc, equal_opportunity_difference
